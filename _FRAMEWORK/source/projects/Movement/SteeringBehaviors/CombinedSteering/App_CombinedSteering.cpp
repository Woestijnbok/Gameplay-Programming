...
{	
	...
	SAFE_DELETE(m_pDrunkSeekBehaviour);
...
void App_CombinedSteering::Start()
{
	...
	m_pDrunkSeekBehaviour = new BlendedSteering({
		BlendedSteering::WeightedBehavior{m_pDrunkWander, 0.5f},
		BlendedSteering::WeightedBehavior{m_pDrunkSeek, 0.5f}
		});

	m_pAgent1 = new SteeringAgent();
	m_pAgent1->SetSteeringBehavior(m_pDrunkSeekBehaviour);
	m_pAgent1->SetMaxLinearSpeed(15.0f);
	m_pAgent1->SetAutoOrient(true);
	m_pAgent1->SetMass(1.0f);
...
void App_CombinedSteering::Update(float deltaTime)
{
	...
	//UI
	{
		...
		ImGui::SliderFloat("Wander", &m_pDrunkSeekBehaviour->GetWeightedBehaviorsRef()[0].weight, 0.f, 1.f, "%.2");
		ImGui::SliderFloat("Seek", & m_pDrunkSeekBehaviour->GetWeightedBehaviorsRef()[1].weight, 0.f, 1.f, "%.2");
