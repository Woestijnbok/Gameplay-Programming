...
void App_PathfindingAStar::Render(float deltaTime) const
{
	...
	m_GraphRenderer.RenderGraph(m_pTerrainGraph, m_bDrawGrid, m_bDrawNodeNumbers, m_bDrawConnections, m_bDrawConnectionsCosts);
...
{
	if (m_startPathId != invalid_node_id && m_endPathId != invalid_node_id && m_startPathId != m_endPathId)
	{
		//BFS pathfinder{ m_pTerrainGraph };
		AStar pathfinder{ m_pTerrainGraph, m_heuristicFunction };

		GraphNode* startNode = m_pTerrainGraph->GetNode(m_startPathId);
		GraphNode* endNode = m_pTerrainGraph->GetNode(m_endPathId);
