...
void App_FSM::Start()
{
	...
	m_pIsTargetVisibleCondition = new IsTargetVisible{};
	m_pIsTargetNotVisibleCondition = new IsTargetNotVisible{};
	m_pIsSearchingTooLongCondition = new IsSearchingTooLong{};

	//4. Create the finite state machine with a starting state and the blackboard
	m_pFiniteStateMachine = new FiniteStateMachine{ m_pPatrolState, pBlackboard };

	//5. Add the transitions for the states to the state machine
	m_pFiniteStateMachine->AddTransition(m_pPatrolState, m_pChaseState, m_pIsTargetVisibleCondition);
	m_pFiniteStateMachine->AddTransition(m_pChaseState, m_pSearchState, m_pIsTargetNotVisibleCondition);
	m_pFiniteStateMachine->AddTransition(m_pSearchState, m_pPatrolState, m_pIsSearchingTooLongCondition);
	m_pFiniteStateMachine->AddTransition(m_pSearchState, m_pChaseState, m_pIsTargetVisibleCondition);
	...
	m_pGuard->SetDecisionMaking(m_pFiniteStateMachine);
}
